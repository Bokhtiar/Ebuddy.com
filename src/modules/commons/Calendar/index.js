...
export default function Calendar(params) {
    const {tasks, ProjectStatus, reviewLagent, handelViewType, handelDateRange, nonBusiness} = params;
            ...
            <Row className='px-2'>
                ...
                <Col span={6} style={{textAlign: "center"}}>
                    <Flex space="1rem" justify="normal">
                        <Select allowClear={true} style={{'marginRight': '1rem'}} placeholder='Year' defaultValue={selectedYear} onChange={changeSelectedYear}>
                            {
                                years.map(y => <Select.Option key={y} value={y}>{y}</Select.Option>)
                            }
                        </Select>
                        <Select allowClear={true} placeholder='Months' defaultValue={selectedMonth} onChange={changeSelectedMonth}>
                            {
