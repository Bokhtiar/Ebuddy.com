...
export default function MonthView({milestones, ProjectStatus, selectedYear, selectedMonth, handelDateRange}) {
    ...
    const popoverContent = (content) => {
       return content.map(c => <li>
                <span class="ant-badge ant-badge-status ant-badge-not-a-wrapper">
                    <span class="ant-badge-status-dot ant-badge-status-warning" style={{background: c.milestone_status_color}}></span>
                    <span class="ant-badge-status-text">
                        <strong class="ml-1">{c?.project_name || ''}</strong><br />
                        <small class="ml-3">{c?.milestone || ''}</small>
                    </span>
                </span>
            </li>)
    };
    ...
    const manageTaskActivityList = (week, day) => {
        if (milestones && milestones.length) {
            ...
            let activities = milestones.filter(t => t.plan_end_date === date); 
                ...
                return <> 
                    {firstPart.map(a => <li>
                        ...
                        <Popover content={popoverContent(activities)} title={moment(date, "YYYY-MM-DD").format('dddd DD MMMM, YYYY')}>
                            <span class="ant-badge-status-dot ant-badge-status-warning" style={{background: a.milestone_status_color}}></span>
                            <span class="ant-badge-status-text">
                                {/* {a.project_code || ''} */}
                                 {/* <br/> */}
                                {/* <p class="ml-3">Project Name: {a?.project_name || ''}</p>
                                <p class="ml-3">Milestone Name: {a?.milestone || ''}</p> */}
                                <strong class="ml-1" style={{fontSize: '9px'}}>{a?.project_name || ''}</strong><br />
                                <small class="ml-3" style={{fontSize: '9px'}}>{a?.milestone || ''}</small>
                            </span>
                        </Popover>
                        </span>
                    </li>)}
                    {
                        activities.length > 1 ? <li>
                                <span className="ant-badge ant-badge-status ant-badge-not-a-wrapper" style={{fontSize: '8px'}}>
                                    <span className="ant-badge-status-text">
                                    <Popover content={popoverContent(activities)} title={moment(date, "YYYY-MM-DD").format('dddd DD MMMM, YYYY')}>
                                        <Button className="pl-0" type="link" style={{fontSize: '9px'}}>+{activities.length - 1} more</Button>
                                    </Popover>
                                    </span>
                                </span>
                            </li> : ''
