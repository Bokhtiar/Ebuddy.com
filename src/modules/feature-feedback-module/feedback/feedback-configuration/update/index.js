...
import { 
  FFM_FEEDBACK_CONFIGURATION_BY_ID,
...
const FeedbackConfigurationUpdate = Form.create()(({form}) => {
  ...
  const [feedbackConfigData, setFeedbackConfigData] = useState();
  ...
  let paramsFeedbackConfigId = params.get('feedback_configuration_id');
  let feedbackConfigId = paramsFeedbackConfigId ? paramsFeedbackConfigId * 1 : undefined;

  const getFeedbackConfigById = async (id) => {
    let url = FFM_FEEDBACK_CONFIGURATION_BY_ID + "/" + id;
    let res = await getData(url);

    if (res) {
      let masterData = res?.data?.data;
      if (res) {
        setFeedbackConfigData(masterData);
        setFeedbackConfiguration(masterData?.ffm_feedback_options_configs);
      }
    }
  }
  ...
  useEffect(() => {
    ...
    getFeedbackList();
    if(!feedbackConfigId){
      setFeedbackConfiguration([{ id: uuid() }]);
    }
  }, []);
  ...
  useEffect(() => {
    if(feedbackConfigData?.ffm_feedback_id){
      let result = feedbackList?.find(item => item.id === feedbackConfigData?.ffm_feedback_id);
      if(result){
        setFeedbackTitle(result.title);
      }
    }
  ...
  useEffect(() => {
    if(feedbackConfigId) {
      getFeedbackConfigById(feedbackConfigId);
      getProjectList();
      getServiceList();
      getFeedbackList();
      getFeatureList();
  ...
  const localSubmit = (e) => {
    ...
    form.validateFields(async(err, values) => {
      if (!err) {
        setLoading(true);
        let configData = [];
        if(feedbackConfigId && feedbackConfiguration){
          feedbackConfiguration.forEach((config, index) => {
            let data = {
              "id": config.id && isNaN(config.id) === false ? config.id : null,
              ...
              "reasons": [
                {
                  "id": config.ffm_feedback_reasons_configs && isNaN(config.ffm_feedback_reasons_configs[0]?.id) ? values[`options-${config.ffm_feedback_reasons_configs[0]?.id}`] : null,
  ...
  return (
    <Wrapper>
      <PageHeader
        ...
        title="Feedback Configuration Update"
      />
      <Form 
        className="m-4"
          ...
          <Col span={12}>
            <Form.Item label={'Project Name'}>
              {form.getFieldDecorator('project_id', {
                initialValue: feedbackConfigData?.project_id ? feedbackConfigData?.project_id : undefined
              })(<Select 
                allowClear
                size="large"
                showSearch
          ...
          <Col span={12}>
            <Form.Item label={'Service Name'}>
              {form.getFieldDecorator('ffm_service_id', {
                initialValue: feedbackConfigData?.ffm_service_id ? feedbackConfigData?.ffm_service_id : undefined
              })(<Select 
                allowClear
                size="large"
                showSearch
          ...
          <Col span={12}>
            <Form.Item label={'Service Module Name'}>
              {form.getFieldDecorator('ffm_module_id', {
                initialValue: feedbackConfigData?.ffm_module_id ? feedbackConfigData?.ffm_module_id : undefined
              })(<Select 
                allowClear
                size="large"
                showSearch
          ...
          <Col span={12}>
            <Form.Item label={'Feature Name'}>
              {form.getFieldDecorator('ffm_feature_id', {
                initialValue: feedbackConfigData?.ffm_feature_id ? feedbackConfigData?.ffm_feature_id : undefined
              })(<Select 
                allowClear
                size="large"
                showSearch
          ...
          <Col span={24}>
            <Form.Item label={'Feedback Name'}>
              {form.getFieldDecorator('ffm_feedback_id', {
                initialValue: feedbackConfigData?.ffm_feedback_id ? feedbackConfigData?.ffm_feedback_id : undefined
              })(<Select 
                allowClear
                size="large"
                showSearch
                  ...
                  <Col span={12}>
                    <Form.Item label={'Feedback Options'}>
                      {form.getFieldDecorator(`options-${item.id}`, {
                        initialValue: item?.ffm_option_id ? item?.ffm_option_id : undefined
                      })(<Select 
                        allowClear
                        size="large"
                        showSearch
                  ...
                  <Col span={12}>
                    <Form.Item label={'Feedback Reasons'}>
                      {form.getFieldDecorator(`reasons-${item.id}`, {
                        initialValue: item?.ffm_feedback_reasons_configs ? item?.ffm_feedback_reasons_configs[0]?.ffm_reason_id : undefined
                      })(<Select 
                        allowClear
                        size="large"
                        showSearch
...
export default FeedbackConfigurationUpdate;