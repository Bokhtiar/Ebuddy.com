...
const EditProject = Form.create()(({ form, milestoneFields, setModal, projectId }) => {
    ...
    const getProjectDetails = async () => {
        let url = PROJECT_SETUP_VIEW + projectId;
        let res = await getData(url);

        if (res) {
            let masterData = res?.data?.data;
            setProjectDetails(masterData);
            setMilestoneList(masterData?.milestones);
            getClientPOCList(masterData?.client?.id);
        }
    };
    ...
    const localSubmit = (e) => {
        ...
        form.validateFields(async (err, values) => {
            if (!err) {
                ...
                let url = PROJECT_TRACKER_UPDATE + "/" + projectId;
    ...
    return (
        ...
        >
                ...
                <Col span={0}>
                    <Form.Item label={''}>
                        {form.getFieldDecorator('client_id', {
                            initialValue: projectDetails?.client?.id ? projectDetails?.client?.id : undefined
                        })(<Input placeholder="Enter Client Name" size="large" hidden />)}
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item label={'Client Name'}>
                        {form.getFieldDecorator('client_name', {
                            initialValue: projectDetails?.client?.name ? projectDetails?.client?.name : undefined
                        })(<Input placeholder="Enter Client Name" size="large" disabled />)}
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item label={'Client Type'}>
                        {form.getFieldDecorator('client_type', {
                            initialValue: projectDetails?.client_type ? projectDetails?.client_type : undefined
                        })(<Select
                            allowClear
                            size="large"
                            showSearch
                ...
                <Col span={12}>
                    <Form.Item label={'Brand Name'}>
                        {form.getFieldDecorator('brand', {
                            initialValue: projectDetails?.brand?.name ? projectDetails?.brand?.name : undefined
                ...
                <Col span={12}>
                    <Form.Item label={'KAM'}>
                        {form.getFieldDecorator('kam_id', {
                            initialValue: projectDetails?.kam?.emp_id ? projectDetails?.kam?.emp_id : undefined
                        })(<Select
                            allowClear
                            size="large"
                            showSearch
                ...
                <Col span={12}>
                    <Form.Item label={'Service'}>
                        {form.getFieldDecorator('service_id', {
                            initialValue: projectDetails?.service_id ? projectDetails?.service_id : undefined
                        })(<Select
                            allowClear
                            size="large"
                            showSearch
                ...
                <Col span={12}>
                    <Form.Item label={'Client POC'}>
                        {form.getFieldDecorator('client_poc', {
                            initialValue: projectDetails?.client_poc?.id ? projectDetails?.client_poc?.id : undefined
                        })(<Select
                            allowClear
                            size="large"
                            showSearch
                ...
                <Col span={12}>
                    <Form.Item label={'Decision Maker Name'}>
                        {form.getFieldDecorator('decision_maker_poc_id', {
                            initialValue: projectDetails?.decision_maker_poc_id ? projectDetails?.decision_maker_poc_id : undefined
                        })(<Select
                            allowClear
                            size="large"
                            showSearch
                ...
                <Col span={12}>
                    <Form.Item label={'Value'}>
                        {form.getFieldDecorator('value', {
                            initialValue: projectDetails?.value ? projectDetails?.value : undefined
                ...
                <Col span={24}>
                    <Form.Item label={'Project Name'}>
                        {form.getFieldDecorator('name', {
                            initialValue: projectDetails?.name ? projectDetails?.name : undefined
                        })(<Input
            ...
            <h4>Milestones</h4>
            <Divider />
            {milestoneList?.length > 0 ? milestoneList?.map((item, index) => {
                return (
                    <Row gutter={16} key={`milestones-${item.id}`}>
                        <Col span={2}>
                            <Form.Item required={false} label={'Serial'}>
                                <InputNumber placeholder="Serial" min={1} max={milestoneList.length} defaultValue={item?.serial ? item?.serial : index+1} onChange={(e) => {
