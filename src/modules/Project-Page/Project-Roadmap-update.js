...
const ProjectRoadmapUpdate = (props) => {
  ...
  const checkPemissions = async () => {
    ...
    if (permissions && permissions.length) {
        ...
        if (permNames.includes('Can delete milestone')) {
  ...
  let columns = [
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <span>
            {row.actual_start_date ? row.actual_start_date : ""}
            <Button
              type="link"
              icon="calendar"
              disabled={row.demo}
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <span>
            {row.actual_end_date ? row.actual_end_date : ""}
            <Button
              type="link"
              icon="calendar"
              disabled={row.demo}
  ...
  const updateMilestone = async (type, context) => {
    ...
    if (selectMileStone && selectMileStone.id) {
      ...
      // let updateURL = PROJECT_MILESTONE_UPDATE;
      // let removeURL = PROJECT_MILESTONE_DATE_UPDATE;

      // let res = context === 'remove' ? await postData( removeURL + selectMileStone.id, updateData) : await postData( updateURL + selectMileStone.id, updateData);
    ...
    <>
            ...
            <Table
              ...
              columns={
                milestoneRoadmapStatus === 'Non-commercial'
                  ? 
                  (
                    columns.filter(col => col.key !== 'PAYMENT' && col.key !== 'payment_status_id' && col.key !== 'payment_method_id')
                  ):columns
              }
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("due-date", "update")}
          >
            {" "}
            Update{" "}
          </Button>,
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("plan_start_date", "update")}
          >
            {" "}
            Update{" "}
          </Button>,
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("plan_end_date", "update")}
          >
            {" "}
            Update{" "}
          </Button>,
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("actual_start_date", "update")}
          >
            {" "}
            Update{" "}
          </Button>,
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            type="danger"
            loading={loading}
            onClick={() => updateMilestone("actual_end_date", 'remove')}
          >
            {" "}
            Remove{" "}
          </Button>,
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("actual_end_date", "update")}
          >
            {" "}
            Update{" "}
          </Button>,
      ...
      >
        <DatePicker
          allowClear={false}
          disabledDate={(current) => {
            let customDate = moment(selectMileStone.actual_start_date).format("YYYY-MM-DD");
            return current && current < moment(customDate, "YYYY-MM-DD");
          }}
          value={
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            type="primary"
            loading={loading}
            onClick={() => updateMilestone("milestone-status", "update")}
          >
            Update
          </Button>,
        ]}
      ...
      <Modal
        ...
        title="Payment"
        onCancel={() => {setPaymentModal(false); setLoading(false); setSelectMilestone(null)}}
        footer={[
          <Button
            key="submit"
            type="primary"
      ...
      >
        <InputNumber
          size="large"
          style={{ width: "100%" }}
          value={selectMileStone?.payment || undefined}
...
export default ProjectRoadmapUpdate;
