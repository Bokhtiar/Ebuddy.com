...
const actionCBSStatus = ["Approved By Finance", "Declined By Finance"];
...
const ApproveBills = Form.create()(({ form }) => {
  ...
  const rowSelection = {
    ...
    getCheckboxProps: (record) => ({
      disabled: record.status !== "Approved By Dept", // Column configuration not to be checked
      name: record.status,
    }),
  };
  ...
  const getApprovedList = async () => {
    let url = CBS_APPROVAL_LIST_FINANCE + "?";
  ...
  const updateApprovalStatus = (e) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        if (
          values?.status === "Declined By Finance" &&
  ...
  const ApprovedByFinance = async (value) => {
  ...
  return (
    <Wrapper>
      ...
      <Button
        ...
        // size="large"
        onClick={() => {
          setMultipleStatusUpdateModal(true);
        }}
      ...
      >
        <Select
          ...
          dropdownMatchSelectWidth={false}
          value={selectEmploy}
          onChange={(value) => ApprovedByFinance(value)}
          filterOption={(input, option) =>
            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >=
            0
          }
