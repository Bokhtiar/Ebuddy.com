...
import { PA_CONFIG_SUMMARY, 
  ...
  PA_CATEGORY_LIST, 
  PA_CRITERIA_CONFIGURATION_UPDATE,
  PA_SUB_CRITERIA_CONFIGURATION_UPDATE,
...
export default ({context}) => {
  ...
  const [criteriaUpdated, setCriteriaUpdated] = useState(false);
  ...
  const thirdSubmit = async (data) => {
    ...
    let res = await postData(PA_CRITERIA_CONFIGURATION_UPDATE, {...data});

    if (res) {
      let masterData = res.data.data;
      alertPop("success", "Process completed successfully!");
      setCriteriaUpdated(true);
    } else {
      setCurrent(current - 1);
    }
  ...
  const fourthSubmit = async (data) => {
    let res = await postData(PA_SUB_CRITERIA_CONFIGURATION_UPDATE, {...data});
      ...
      <h3 className="p-4"><strong>PA Configure</strong></h3>
        ...
        :steps[current].id === 4 ? */}
          <StepFour 
            ...
            setSubCriteriaList={setSubCriteriaList}
