...
const ChartView = ({employeePAData}) => {
        ...
        tempArray.push(
          {
            'name': 'TOTAL',
            'time': item.month,
            'score': Number(((item.pa_emp_categories[0].emp_score * 4) / 100) + (item.pa_emp_categories[1].emp_score * 1) / 100).toFixed(2),
          },
        )
      })
    }
  ...
  return (
    <Line {...config} style={{backgroundColor: 'white', height: 'calc(100vh - 17rem)'}}/>
  );
};
