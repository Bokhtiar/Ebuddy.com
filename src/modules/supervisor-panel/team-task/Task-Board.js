...
import { Card, Tag, Progress, Icon, Row, Col } from 'antd';
...
const TaskBoard = (props) => {
    const {detailsModalShow, useActivity, updateActionStatus, nonBusiness, setRowData} = props;
    ...
    const onDragEnd = (result, columns, setColumns) => {
        ...
        if (source.droppableId !== destination.droppableId) {
            ...
            if (activity) {
                activity.activity_status = destination.droppableId;
    ...
    const milestoneCount = (items) => {
        return `${items.length || 0} ${items.length < 2 ? 'Item' : 'Items'}`;
    }

    return (
        <div className="sc-dKEPtC" style={{height: "72%" }}>
            <div className="sc-dBAPYN">
                <div className="sc-sdgfhjsdg">
                    <div className="sc-wrwerwe">
                        <section className="sc-section">
                            <div className="sc-contentrr">
                                  {/* style={{ display: "flex", height: "100%", marginLeft: '1rem', marginTop: '1rem' }} */}
                                <DragDropContext
                                                            ...
                                                            {(provided, snapshot) => {
                                                                return (
                                                                    ...
                                                                    <div ref={provided.innerRef}
                                                                        ...
                                                                        <Card style={item.isOverDue && (column.name === 'To-Do' || column.name === 'WIP' || column.name === 'HOld') ? {border: '2px solid red'} : {border: 'none'}} className="mb-0">
                                                                            <span>{item?.activity_Priority?.name || 'N/A'}</span>
                                                                            <h4 className="mt-1 mb-2">{item.title}</h4>
                                                                            <p>{item?.project_details?.name || 'N/A'}</p>
                                                                            {nonBusiness ?
                                                                                item?.function_activity?.name ?  
                                                                                    <Tag color="purple" className="mr-4">{item?.function_activity?.name}</Tag>
                                                                                    : null
                                                                            :
                                                                                item?.milestone?.full_name ?  
                                                                                    <Tag color="purple" className="mr-4">{item?.milestone?.full_name}</Tag>
                                                                                    : null
                                                                            }
                                                                            {/* <Flex justify="normal">
                                                                                <Tag color="purple" className="mr-4">{item?.milestone?.full_name}</Tag>
                                                                                <span className="mr-4">
                                                                                    {item.start_date} {item.start_date ? <br/> : ''} 
                                                                                    {item.due_date}
                                                                                </span>
                                                                                <Progress percent={item.milestone_progress} showInfo={false} style={{width: '30%'}} />
                                                                            </Flex> */}
                                                                            {/* <Flex justify="normal" className="mt-4">
                                                                                <span className="mr-4"><span className="text-primary"># </span>{item?.id}</span>
                                                                                <img src={demoUser} width="25" height="25" className="mr-4"/>
                                                                            ...
                                                                            <Row gutter={8}>
                                                                                ...
                                                                                <Row gutter={8}>
                                                                                    <Col span={12}>
                                                                                        <p style={{margin: "10px 0px"}}>{item?.repeats}</p>
                                                                                    </Col>
                                                                                    <Col>
                                                                                    {
                                                                                        item?.reviewRating?.name ? <span style={{color: item?.reviewRating?.color, margin: "0 10px", float: 'right'}}>
                                                                                            {item?.reviewRating?.name}
                                                                                        </span> : ''
                                                                                    }
                                                                                    {
                                                                                        item.isOverDue && (column.name === 'To-Do' || column.name === 'WIP' || column.name === 'Hold')  ? <span style={{color: 'red', margin: "0 10px", float: 'right'}}>Overdue</span> : ''
                                                                                    }
                                                                                    </Col>
                                                                                </Row>                                                                                                                                                            
                                                                            </div>
