...
import { PIS_COMPANY_LIST_All, SERVICE_SETUP_DROPDOWN_LIST, GET_SERVICE_TYPE_LIST, USER_WISE_FEEDBACK_LIST } from "../../../../../scripts/api";
import { dateFormat } from "../../../../../scripts/helper";

const FeedbackSubmissionList = () => {
    ...
    const getPitchList = async () => {
      let url = USER_WISE_FEEDBACK_LIST + '?';
      ...
      },
      {
        ...
        render: (row) => (
          <Button onClick={() => {
            ...
            setModal(true);
            setIsUpdate(true);
            history.push(`/feedback/my-feedback-card-wise-question-list?feedback_id=${row?.id}`)
          }} type="link">
            Update
          </Button>
        ),
        ...
        <Wrapper>
            <Flex space="1rem" justify="normal">
                <Button 
                    ...
                    onClick={()=>
                      history.push('/feedback/my-feedback-submission-initiate')
                    } 
                    width="40%" 
                    type="primary" 
                    style={{'marginRight': '1rem'}}
                >
                  Setup New Feedback Submission
                </Button>
                <Select 
                  allowClear
                ...
                >
                  {companyList ? companyList.map(com=>{
                    return(
                      <Select.Option key={com.id} value={com.id} label={com.name}>{com.name}</Select.Option>
                    )
                  }):null}
                </Select>
                <Select 
