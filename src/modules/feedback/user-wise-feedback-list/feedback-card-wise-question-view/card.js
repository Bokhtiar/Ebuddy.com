...
const Card = Form.create()(({ form, cardData, setCurrent, current, dataLength, answerData, setAnswerData, cardWiseQuestionList, feedbackId, typeCheck }) => {
        ...
        form.validateFields(async (err, values) => {
            if (!err) {
                ...
                if(typeCheck === 'view'){
                    ...
                    if(current === dataLength) history.push('/feedback/user-wise-feedback-list');
                }
                else{
                    let cardDataArray = [];
                        ...
                        {cardData?.questions?.map(item => {
                            return (
                                <Collapse
                                    key={item.id}
                                    className="m-2"
                                    defaultActiveKey={cardData?.questions?.map(item => item.feedback_question?.id)}
                                // onChange={callback}
                                >
