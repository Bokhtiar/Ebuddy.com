...
const BulkImport = Form.create()(({ form, props }) => {
  ...
  const fileUploadHandler = async (event) => {
    ...
    document.getElementById('leave-bulk-import').innerHTML=value.name;
    setFileUpload(value);
  }
  ...
  const submit = () => {
    ...
    postData(url, formData).then(res => {
      ...
      document.getElementById('leave-bulk-import').innerHTML="";
      setFileUpload();
    });
  };
      ...
      <div className="m-4">
        <div style={{ textAlign: 'right' }}>
          <Button 
            size='large' 
            type="link" 
            onClick={()=> window.open(process.env.REACT_APP_BASE + 'upload-formats/bulk_leave_entry.xlsx')}
          >
        ...
        <div className="file-upload-content" style={{margin: '1rem 0'}}>
          ...
          <div className="file-upload-wrapper" data-text="">
              <span className="attacment-filename" id="leave-bulk-import"></span>
              <input 
