...
import { Input, Col, Row, Select, Card, Collapse, Button } from "antd";
...
export default function ActivityDetailsModal({ updateEvent, rowData, updatedStatus, boardViewStatusFlag, setStatus}) {
    ...
    const getDetailsData = async () =>{
        let res = await getData(SOP_USER_ACTIVITY_DETAILS + `/${activityId}`);
        if (res) {
            let masterdata = res?.data?.data;
            setTaskData(masterdata);
            if((masterdata?.status === 'Pending') && (profile?.emp_id == masterdata.assignee?.emp_id)) setShowButton(true);
    ...
    useEffect(()=>{
        if(rowData?.id || commentFlag || statusFlag || review || rating) getDetailsData();
    },[rowData, commentFlag, statusFlag, review, rating]);

    return (
        <Wrapper style={{margin: '4rem 1rem', width: '99%'}}>
            ...
            <Row gutter={32}>
                <Col span={17} style={{borderRight: '1px solid #ccc'}}>
                    ...
                    {showButton && <div className="mt-3">
                        <Button type="primary" onClick={() => notiReject()}> Reject </Button>
                        <Button type="primary" style={{float: "right"}} onClick={() => notiAccept()}> Accept </Button>
                    </div>}
                </Col>
