...
import { CREATE_GET_BIZ_TASKS } from "../../scripts/api";
...
class MyTask extends Component {
  ...
  componentDidMount() {
    this.view(
      `${CREATE_GET_BIZ_TASKS}?page=${this.props.params.page}&status=${
  ...
  filter = filters => {
    if (filters) {
      this.setState(
        ...
        () => {
          const filtered_query = `${CREATE_GET_BIZ_TASKS}?page=1&from_date=${
