...
import {  PROJECT_LIST_BY_KAM, PROJECT_SUMMARY_LIST_BY_KAM, MILESTONE_STATUS_LIST, REVIEW_STATUS_LISTVIEW} from "../../../scripts/api";
...
export default function ProjectSummaryDetails() {
    ...
    const getProjectMilestone = async () => {
        let res = await getData(PROJECT_SUMMARY_LIST_BY_KAM + '?project_id=' + projectId);
    ...
    const columns = [
        {
            ...
            render: (milestone, row) => (
                ...
                // </div>
                <Link target="_blank" to={'/project-management/task-details?milestone=' + milestone?.milestone_id + '&projectId=' + row?.project_id + `&empId=${kamId}`} style={{textDecoration: 'underline', color:'#5BB7FF'}}>
    ...
    return (
        ...
        <Wrapper>
            <PageHeader
                ...
                onBack={() => history.push('/project-management/project-summary' + "/" + "&kam=" + kamId)}
