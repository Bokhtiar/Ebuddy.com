...
import {Input, Radio, Col, Row, Select, DatePicker, Checkbox, Form, Icon, Button, Typography, Empty, InputNumber, PageHeader} from "antd";
...
export default function SubTaskActivityCreate({setModal, activityDetails, isTaskUpdate, updateEvent, projectId, nonBusiness, setNonBusiness, flag, modal, resetCount}) {
    ...
    let paramsActivity = params.get('activity_id');
    let delegate = params.get('delegate');
    let subTask = params.get('sub_task');
    ...
    const submit = (value, form) => {
        ...
        if(value?.department_id) formData.append("department_id", value?.department_id);
        if(value?.function_type_id) formData.append("function_type_id", value?.function_type_id);
        ...
        if(value?.project_milestone_id) formData.append("project_milestone_id", value?. project_milestone_id);
        if(value?.title) formData.append("title", value?.title);
        if(value?.parent_activity_id) formData.append("parent_activity_id", value?.parent_activity_id);
        if(value?.activity_type_id) formData.append("activity_type_id", value?.activity_type_id);
        // if(value?.repeats) formData.append("repeats", value?.repeats);
        if(value?.activity_priority_id) formData.append("activity_priority_id", value?.activity_priority_id);
        if(value?.assignee) formData.append("assignee", value?.assignee);
        if(value?.communication_medium) formData.append("communication_medium", value?.communication_medium);
        if(value?.client_department_id) formData.append("client_department_id", value?.client_department_id);
        if(value?.note) formData.append("note", value?.note);
        if(value?.estimated_time) formData.append("estimated_time", value?.estimated_time);
        if(delegate){
            if(delegate === 'yes') formData.append("delegated", 1);
            else formData.append("delegated", 0);
        } 
        
        if(fileUpload) {
            if(fileUpload.size > 2097152) {   //2097152 = 2MB
                alertPop('error', "File size is greater than 2MB.");
                setLoading(false);
                return ;
            }
            else formData.append("attachment", fileUpload);
        }

        if(activityRepeat !== undefined) formData.append("repeats", value?.repeats || null);
        formData.append("reporter", dropdownData?.reporter?.emp_id || activityDetails?.reporter_user?.emp_id);
        formData.append("status", isTaskUpdate ? activityDetails?.activity_status || 'To-Do' : 'To-Do');
        formData.append("start_date", value.start_date.format('YYYY-MM-DD') + '');
        formData.append("due_date", value.due_date.format('YYYY-MM-DD') + '');
        if(activityTillDate) formData.append("repeats_till_date", value.repeats_till_date.format('YYYY-MM-DD') + '');
        ...
        postData(url, formData).then(res => {
            if (res?.data?.code === 201) {
                ...
                setLoading(false);
                setFileUpload();
                if(fileUpload) document.getElementById('activity-attachemnt-name').innerHTML=null;
    ...
    const dropDownDataSet = (url, key) => {
        getData(url).then(res => {
            ...
            if (key) {
                ...
                // console.log('[dropdown-data-set]', res?.data?.data?.projects);
                setDropdownData((preState) => ({
                    ...preState,
                    ...res?.data?.data || {}
                }));
    ...
    const methods = {
        ...
        functionType: (e) => {
            dropDownDataSet(ACTIVITY_FUNCTION_DROPDOWNLIST + '?function_type_id=' + e, 'activityFunctionList');
            if(e && nonBusiness){
    ...
    const departmentChangeHandel = (value) => {
        ...
        getAssignee(value);
        dropDownDataSet(FUNCTION_TYPES_DROPDOWNLIST + '?department_id=' + value, 'functionTypeList');
    }

    useEffect(()=>{
        if(activityDetails?.contact_person?.client_id) clientManage(activityDetails?.contact_person?.client_id)
    ...
    useEffect(()=>{
        if(modal && !isTaskUpdate) dropDownDataSet(FUNCTION_TYPES_DROPDOWNLIST + '?department_id=' + userInfo?.department_id, 'functionTypeList');
    ...
    useEffect(() => {
        ...
        if(isTaskUpdate && activityDetails) setIsCommunication(activityDetails?.is_communication);
    ...
    useEffect(() => {
        if(nonBusiness && userInfo) {
            ...
            if(isTaskUpdate && activityDetails?.department_id){
                ...
                dropDownDataSet(FUNCTION_TYPES_DROPDOWNLIST + '?department_id=' + activityDetails?.department_id, 'functionTypeList');
            }
            //for logged in user in create
            else if(userInfo?.department_id){
                getAssignee(userInfo?.department_id);
                dropDownDataSet(FUNCTION_TYPES_DROPDOWNLIST + '?department_id=' + userInfo?.department_id, 'functionTypeList');
            }
        }
    ...
    useEffect(() => {
        if ( typeof activityDetails === 'object' && !activityDetails?.reporter_user) {
            getUser();
        }
    }, [activityDetails])
    ...
    return <CreateForm submit={submit} methods={methods} dropdownData={dropdownData} projectId={projectId}loading={loading} activityDetails={activityDetails} isUpdateState={isTaskUpdate} userInfo={userInfo} nonBusiness={nonBusiness} fileUploadHandler={fileUploadHandler} setActivityRepeat={setActivityRepeat} activityRepeat={activityRepeat} setDropdownData={setDropdownData} modal={modal} flag={flag} clientManage={clientManage}pOCList={pOCList} departments={departments} setDepartments={setDepartments} setdepartmentSelect={setdepartmentSelect} employeeList={employeeList} DepartmentSelect={DepartmentSelect} dropDownDataSet={dropDownDataSet} isCommunication={isCommunication} setIsCommunication={setIsCommunication} isTaskUpdate={isTaskUpdate} unsetAssignee={unsetAssignee}
    setUnsetAssignee={setUnsetAssignee} delegate={delegate} subTask={subTask} departmentChangeHandel={departmentChangeHandel} clientList={clientList} activityTillDate={activityTillDate} setActivityTillDate={setActivityTillDate} taskData={taskData}/>;
}

const CreateForm = Form.create()(
    
    ({delegate,form, submit, subTask,  loading, editValues, dropdownData, methods, activityDetails, isUpdateState, userInfo, projectId, nonBusiness, fileUploadHandler, setActivityRepeat, activityRepeat, setDropdownData, modal, flag, clientManage, pOCList, departments, setDepartments, setdepartmentSelect, employeeList, DepartmentSelect, dropDownDataSet, isCommunication, setIsCommunication, isTaskUpdate, unsetAssignee, setUnsetAssignee, departmentChangeHandel, clientList, activityTillDate, setActivityTillDate, taskData }) => {
        ...
        const setDate = (value) =>{
            setStartDate(value);
            if(activityRepeat === 'Daily') form.setFieldsValue({due_date: moment(value)})
            if(activityRepeat === 'Weekly') form.setFieldsValue({due_date: moment(value).add(7, 'd')})
            if(activityRepeat === 'Monthly') form.setFieldsValue({due_date: moment(value).add(1, 'M')})
            if(activityRepeat === 'Quarterly') form.setFieldsValue({due_date: moment(value).add(3, 'M')})
            if(activityRepeat === 'Half Yearly') form.setFieldsValue({due_date: moment(value).add(6, 'M')})
            if(activityRepeat === 'Yearly') form.setFieldsValue({due_date: moment(value).add(1, 'y')})
        }
        ...
        const handleDate = (value) =>{
            if(value === undefined) {
                ...
                if(value === 'Daily') form.setFieldsValue({due_date: moment(startDate)})
                if(value === 'Weekly') form.setFieldsValue({due_date: moment(startDate).add(7, 'd')})
                if(value === 'Monthly') form.setFieldsValue({due_date: moment(startDate).add(1, 'M')})
                if(value === 'Quarterly') form.setFieldsValue({due_date: moment(startDate).add(3, 'M')})
                if(value === 'Half Yearly') form.setFieldsValue({due_date: moment(startDate).add(6, 'M')})
                if(value === 'Yearly') form.setFieldsValue({due_date: moment(startDate).add(1, 'y')})
        ...
        useEffect(() => {
            if(dropdownData?.projectList?.length) {
                for(let i=0; i<dropdownData.projectList?.length ; i++){
                    if(dropdownData.projectList[i].name === 'General Activity' && flag !== "project-list"){
                        setProjectID(dropdownData.projectList[i].id);
                        if(!nonBusiness) methods.projectChangeHandle(dropdownData.projectList[i].id);
                    }
                }
            }
        ...
        useEffect(()=>{
            if(activityDetails?.projects?.id && !nonBusiness) methods.projectChangeHandle(activityDetails?.projects?.id);
        ...
        useEffect(()=>{
            if(activityDetails?.projects && !nonBusiness) methods.projectChangeHandle(activityDetails?.projects?.id);
        },[activityDetails])
        ...
        useEffect(() => {      
            if (dropdownData?.activity_types?.length && !projectId && nonBusiness) {
                let findGeneral = dropdownData.activity_types.find(act => act.name === 'General');
                let findDefault = dropdownData.activity_types.find(act => act.default === 1);

                if (findGeneral && projectID === 16) setDefaultActivityType(findGeneral?.id);
        ...
        useEffect(() => { 
            if(isUpdateState){
                form.setFieldsValue({activity_type_id : activityDetails?.activity_type?.id ? activityDetails?.activity_type?.id : activityDetails?.activity_type_formatted?.id ? activityDetails?.activity_type_formatted?.id : defaultActivityType ? defaultActivityType : undefined})
            }   
            // else{
        ...
        return (
            <Wrapper>
                <PageHeader
                    ...
                    title="Deligate / Sub Task Create"
                />
                <div style={{margin: '2rem 0.5rem', width: '99%'}}>
                    ...
                    {(nonBusiness || subTask) ? <>
                        <Row gutter={32}>
                            <Col span={8}>
                                <Form.Item label={'DEPARTMENT NAME'}>
                                    {form.getFieldDecorator('department_id', {
                                        ...
                                        initialValue: subTask ? null : (activityDetails?.department_id ? activityDetails?.department_id : userInfo ? userInfo?.department_id : undefined)
                                    })(<Select 
                                        allowClear={false} 
                                        size="large" 
                            ...
                            <Col span={8}>
                                <Form.Item label={'FUNCTION TYPE'}>
                                    ...
                                    })(<Select getPopupContainer={trigger => trigger.parentNode} placeholder='Select Function Type' size="large" 
                                        onChange={(value) => {methods.functionType(value); form.resetFields('function_id')}} 
                                        showSearch
                                        filterOption={(input, option) =>
                                            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                        }>
                                        {dropdownData?.functionTypeList?.map((item, index) =>
                                            <Select.Option key={`item-${index}-${item.id}`}
                            ...
                            <Col span={8} style={{float: 'right'}}>
                                <Form.Item label={'FUNCTION NAME'}>
                                    {form.getFieldDecorator('function_id', {
                                        rules: [{required: true, message: "Required!"}],
                                        initialValue: activityDetails?.function_activity ? activityDetails?.function_activity?.id : undefined
                                    })(<Select getPopupContainer={trigger => trigger.parentNode} size="large" placeholder='Select function name' showSearch
                                    filterOption={(input, option) =>
                                        option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                    }>
                                        {dropdownData?.activityFunctionList?.map((activityFunction, index) =>
                                            <Select.Option key={`activityFunction-${index}-${activityFunction.id}`}
                                                        value={activityFunction.id}>{activityFunction.name}</Select.Option>
                                        )}
                                    </Select>)}
                                </Form.Item>
                            </Col>
                        ...
                        {!nonBusiness ? <><Row gutter={32}>
                            <Col span={16}>
                                <Form.Item label={'SELECT PROJECT'}>
                                    {form.getFieldDecorator('project_id', {
                                        rules: [{required: subTask ? false : true, message: "Required!",}],
                                        ...
                                        initialValue: subTask ? null : taskData?.projects ? taskData?.projects?.id : console.log('activity-details-project-id NOT_FOUND', taskData)
                                    })(<Select getPopupContainer={trigger => trigger.parentNode} placeholder='Select Project' size="large" onChange={methods.projectChangeHandle} 
                                        showSearch
                                        filterOption={(input, option) =>
                                            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                        }>
                                            
                                        {subTask ? null : dropdownData?.projectList?.map((project, index) =>
                            ...
                            <Col span={8} style={{float: 'right'}}>
                                <Form.Item label={'Select Milestone'}>
                                    {form.getFieldDecorator('project_milestone_id', {
                                        ...
                                        initialValue: activityDetails?.project_milestone_id ? activityDetails?.project_milestone_id : defaultMilestone ? defaultMilestone : undefined
                                    })(<Select getPopupContainer={trigger => trigger.parentNode} size="large" placeholder='Select Milestone' showSearch
                                    filterOption={(input, option) =>
                                        option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                    }>
                                        {dropdownData?.milestones?.map((milestone, index) =>
                                            <Select.Option key={`milestone-${index}-${milestone.id}`}
                                                        value={milestone.id}>{milestone.full_name}</Select.Option>
                                        )}
                                    </Select>)}
                                </Form.Item>
                            </Col>
                        ...
                        <Row gutter={32}>
                            ...
                            <Col span={8}>
                                <Form.Item label={'ESTIMATED TIME (MINUTES)'}>
                                    ...
                                    })(<InputNumber size="large" placeholder="Enter estimated time" style={{width: '100%'}}/>)}
                                </Form.Item>
                            </Col>
                        </Row>
                        <Row gutter={32}>
                            <Col span={0}>
                                <Form.Item label={''}>
                                    {form.getFieldDecorator('parent_activity_id', {
                                        rules: [],
                                        initialValue: taskData?.id ? taskData?.id : undefined
                                    })(<Input placeholder="Enter parent task" size="large" hidden/>)}
                                </Form.Item>
                                </Col>
                                <Col span={8}>
                                <Form.Item label={'PARENT TASK/ACTIVITY'}>
                                    {form.getFieldDecorator('parent_activity_title', {
                                        rules: [],
                                        initialValue: taskData?.title ? taskData?.title : undefined
                                    })(<Input placeholder="Enter parent task" size="large" disabled/>)}
                                </Form.Item>
                                </Col>
                                ...
                                <Col span={8}>
                                    <Form.Item label={'ASSIGNEE'}>
                                        {form.getFieldDecorator('assignee', {
                                            rules: [{required: true, message: "Required!"}],
                                            initialValue: (delegate && delegate == 'no') ? null :  unsetAssignee ? undefined : activityDetails?.assigned_user?.emp_id ? activityDetails?.assigned_user?.emp_id
                                                : activityDetails?.assignee?.emp_id ? activityDetails?.assignee?.emp_id : userInfo?.emp_id ? userInfo?.emp_id : undefined
                                        })(<Select showSearch 
                                            ...
                                            {employeeList?.map((employee, index) =>
                                                <Option key={`employee-${index}-${employee.id}`}
                                                        value={employee.emp_id}>{employee.name}</Option>
                                            )}
                                        </Select>)}
                                    </Form.Item>
                                </Col>
                                <Col span={8}>
                                    <Form.Item label={'REPORTER'}>
                                        {form.getFieldDecorator('reporter', {
                                            rules: [{required: true, message: "Required!"}],
                                            initialValue: dropdownData?.reporter?.name ? dropdownData?.reporter?.name : activityDetails?.reporter_user?.name ? activityDetails?.reporter_user?.name : activityDetails?.reporter?.name ? activityDetails?.reporter?.name : undefined
                            ...
                            <Col span={8}>
                                ...
                                <Col span={8}>
                                    <Form.Item label={'START DATE'}>
                                        ...
                                        })(<DatePicker onChange={setDate} size="large" placeholder="Enter Start Date"/>)}
                                    </Form.Item>
                                </Col>
                                <Col span={8}>
                                    <Form.Item label={'End DATE'}>
                                        ...
                                        })(<DatePicker disabled={activityRepeat ? true : false} size="large" placeholder="Enter End Date"/>)}
                                    </Form.Item>
                                </Col>
                        </Row>
                            ...
                            <Col span={8}>
                                <Form.Item label={'ACTIVITY TYPE'}>
                                    {form.getFieldDecorator('activity_type_id', {
                                        rules: [{required: true, message: "Required!"}],
                                        initialValue: nonBusiness ? undefined : activityDetails?.activity_type?.id ? activityDetails?.activity_type?.id : activityDetails?.activity_type_formatted?.id ? activityDetails?.activity_type_formatted?.id : defaultActivityType ? defaultActivityType : undefined
                                    })(
                                        ...
                                        filterOption={(input, option) =>
                                            ...
                                            { nonBusiness ? 
                                                dropdownData?.activity_types?.filter(act => act.name !== 'Project').map((activity_type, index) =>
                                                    <Select.Option key={`activity_type-${index}-${activity_type.id}`}
                                                                value={activity_type.id}>{activity_type.name}</Select.Option>
                                                )
                                            :
                            ...
                            <Col span={8}>
                                <Form.Item label={'PRIORITY'}>
                                    {form.getFieldDecorator('activity_priority_id', {
                                        rules: [{required: true, message: "Required!"}],
                                        initialValue: activityDetails?.activity_priority?.id ? activityDetails?.activity_priority?.id : activityDetails?.activity_Priority_formatted?.id ? activityDetails?.activity_Priority_formatted?.id : 1
                                    ...
                                    >
                                        {dropdownData?.priority?.map((pr, index) =>
                                            <Select.Option key={`pr-${index}-${pr.id}`}
                                                        value={pr.id}>{pr.name}</Select.Option>
                                        )}
                                    </Select>)}
                                </Form.Item>
                            </Col>
                        ...
                        <Row gutter={32}>
                            <Col span={24}>
                                <Form.Item label={'NOTES/REMARKS'}>
                                    ...
                                    })(<TextArea autoSize={{ minRows: 2, maxRows: 6 }} placeholder="Enter Note"/>)}
                                </Form.Item>
                            </Col>
                        </Row>
                        <hr className="line-top-border"/>
                            ...
                            <Col span={12}>
                                <div className="file-upload-content" style={{marginTop: '1rem'}}>
                                    ...
                                    <div className="file-upload-wrapper" data-text="">
                                        <span className="attacment-filename" id="activity-attachemnt-name"></span>
                                        <input 
